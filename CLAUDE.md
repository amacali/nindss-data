...
- Run the daily history by diagnosis date: `node index.js day` (rolling 30 days) — writes `data/notifications_by_day_diagnostic.json`, ~25s, 67 requests
- Run the daily history by notification date: `node index.js reported` (rolling 30 days) — writes `data/notifications_by_day.json`, ~33s, 67 requests
...
- `data/notifications_by_day_diagnostic.json` — 30 elements, keyed `date` (`YYYY-MM-DD`). A rolling window by DIAGNOSIS_DATE, always rebuilt whole.
- `data/notifications_by_day.json` — the same 30-day window by NOTIFICATION_DATE. The two columns disagree by about 27% over a year, so this is a SECOND dataset, not a replacement — do not sum one against the other, and only `day` reconciles with the month and year files.
